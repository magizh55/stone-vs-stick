...
(function () {
  ...
  btnToss.addEventListener('click', () => {
    ...
    /* Decide winner BEFORE spinning so we can land on the right face */
    const winner = Math.random() < 0.5 ? 1 : 2;
    S.cur = winner;

    /* Front face = Stone (🪨), Back face = Stick (🪵) — always */
    ...
    /*
      Landing angle logic:
      - 0°, 360°, 720° … (multiples of 360) → front face visible → Stone
      - 180°, 540°, 900° … (odd multiples of 180) → back face visible → Stick

      We spin 4 full extra rotations (1440°) for effect, then land on:
        Stone → 1440° + 0°   = 1440°  (even × 180 → front)
        Stick → 1440° + 180° = 1620°  (odd  × 180 → back)
    */
    const landAngle = winner === 1 ? 1440 : 1620;

    /* Reset any previous transform instantly, then animate to landing angle */
    coin.style.transition = 'none';
    coin.style.transform  = 'rotateY(0deg)';
    void coin.offsetWidth; /* force reflow */

    coin.style.transition = 'transform 1.1s cubic-bezier(0.25, 0.1, 0.2, 1)';
    coin.style.transform  = `rotateY(${landAngle}deg)`;

    tossResult.classList.add('hidden');
    coinOverlay.classList.remove('hidden');

    /* Show result text after spin completes (~1.1s) */
    setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      /* Reset coin transform silently for next toss */
      coin.style.transition = 'none';
      coin.style.transform  = 'rotateY(0deg)';
      resetBoard();
      startPlacement();
